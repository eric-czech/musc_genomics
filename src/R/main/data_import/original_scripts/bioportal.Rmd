---
title: "Extracting_Genomic_Data_for_CCLE_From_TCGA_Bioportal"
author: "Stephen Guest"
date: "Friday, December 18, 2015"
output: html_document
---

My efforts to find a source for downloading complete genomic data sets for the CCLE cell lines have been unsuccessful. This file describes the process I used to obtain complete genomic data sets for the CCLE cell lines from
the tcga bioportal using the cgdsr R package.  

First, I generated a list of all of the HUGO human gene symbols by going here: http://www.genenames.org/cgi-bin/download and downloading the information using the default settings.  I then placed the file in my home directory (file is named 'allHUGOgenesymbols') and performed the following commands to get all of the gene symbols, remove withdrawn symbols and get rid of duplicates
...
Opening the cgdsr library and setting up the connection with the server that will be used in subsequent 
cgdsr commands
...
```{r}
# Notes on Cell Lines being used here:
# - Cancer study id is 'cellline_ccle_broad' (#18 per getCancerStudies(mycgds)[18,])
# - Genetic profile used is 'cellline_ccle_broad_CNA' (eg getGeneticProfiles(mycgds, cancerStudy = 'cellline_ccle_broad'))
# - Case List used is 'cellline_ccle_broad_all' (eg getCaseLists(mycgds, cancerStudy = 'cellline_ccle_broad')[2,1:4])
...
Adding the appropriate gene symbols as column names

```{r}
names(Amp_Data)<-all_gene_symbols[1:length(all_gene_symbols)]
```

Adding the appropriate cell line names as row names
...
write the file to the GitHub repository 
...
writing the file to the GitHub repository 

```{r}
write.csv(Exp_Data_NA_Cols_Rows_Removed, file="CCLE_All__Exp_Data_TCGA.csv")
```

Now the mutation data.  We'll again use the cgdsr package and a for loop to extract mutation data for each gene symbol and store it in a dataframe
...
Finally, we'll get the log2 copy number ratios.  We'll again use the cgdsr package and a for loop to extract log 2 copy number data for each gene symbol and store it in a dataframe
...
```{r}
write.csv(Log2_CN_NA_Cols_Removed, file="CCLE_All_Log2ratios_TCGA.csv")
```
